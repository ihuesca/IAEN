...
IAEN <- function(h, ...) {
    ...
    fExport.Action <<- gtkAction(label = "Export", stock.id = "gtk-save")
    gSignalConnect(fExport.Action, "activate", f = export_data_func)
    fExport <<- fExport.Action$CreateMenuItem()
    ...
    Graph1 <<- Graph1.Action$CreateMenuItem()
    ...
    Graph10 <<- Graph10.Action$CreateMenuItem()
    ...
    fExport.Action.Tool <<- fExport.Action$createToolItem()
...
\t\t sna, tnet,\n ggplot2, ggraph,  gplots, scales, reshape, pryr")
    ...
    font.desc <<- function(obj, weight = "normal", style = "normal", size = 12,
        family = "serif", ...) {
    ...
    t.store <<- gtkTreeStore("character")
    tree_view <<- gtkTreeView(t.store)
